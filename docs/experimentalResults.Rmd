...
We have used three multilabel transformation methods : Binary relevance (BR), Label powerset (LP), classifier chain (CC), to transform the dataset into a format, which can be used along existing classification algorithms - Random Forest (RF), k nearest neighbors (KNN), XGboosted trees (XGB).
...
We have trained nine classifaction models over 4 datasets - English tf-idf, English term incidence, German tf-idf, German term incidence and compared the performance of the classifiers, to answer the research questions:  
...
brmodel1 <- br(ds$train, "KNN") #create model - BR +KNN
prediction1 <- predict(brmodel1, ds$test) #prediction for BR +KNN
temp_knn <-
multilabel_evaluate(ds$test, prediction1, "bipartition") #get evaluation metric score  for BR +KNN
...
## RF - Random Forest
brmodel2 <- br(ds$train, "RF") #create model - BR +RF
prediction2 <- predict(brmodel2, ds$test) #prediction for BR +RF
temp_rf <-
multilabel_evaluate(ds$test, prediction2, "bipartition") #get evaluation metric score for BR +RF
...
brmodel3 <- br(ds$train, "XGB") #create model - BR +XGB
prediction3 <- predict(brmodel3, ds$test) #prediction for BR +XGB
temp_xgb <-
multilabel_evaluate(ds$test, prediction3, "bipartition") #get evaluation metric score  for BR +XGB
...
Unlike the experiments conducted on the English dataset, LP combined with KNN deliverd slightly higher performance than the model combining the LP with Random Forest.
...
####<span class="sub-header">Dataset: English, Balanced labelsets</span>
