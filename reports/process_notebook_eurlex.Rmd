...
if(!file.exists(paste(tfidfFileName, 1,".arff", sep = "")) || !file.exists(paste(tfidfFileName, 1,".xml", sep = "")))
...
```{r cls_red_labels_de , message=TRUE, include=TRUE, eval=FALSE, warning= FALSE  }
...
ggplot(perf_metric_de)+geom_tile(aes(x=classificationModel,y=classifier,fill=value),color="black")+geom_text(aes(x=classificationModel,y=classifier,label=round(value,3)),color="black")+scale_fill_distiller(palette = "Spectral")
```
![](../docs/Figs/german_red_lp_tfidf.png)
![](../docs/Figs/german_red_lp_tfidf2.png)
...
performance_lp_rf[["accuracy"]] <- 0.353
performance_lp_rf[["F1"]] <- 0.429
performance_lp_rf[["hamming-loss"]] <- 0.012
performance_lp_rf[["macro-AUC"]] <-0.997
performance_lp_rf[["macro-F1"]] <-0.269
performance_lp_rf[["macro-precision"]] <- 0.276
  performance_lp_rf[["macro-recall"]] <- 0.311
  performance_lp_rf[["micro-AUC"]] <- 0.996
  performance_lp_rf[["micro-F1"]] <- 0.451
  performance_lp_rf[["micro-precision"]] <- 0.449
  performance_lp_rf[["micro-recall"]] <- 0.454
  performance_lp_rf[["precision"]] <- 0.431
  performance_lp_rf[["recall"]] <-0.438
  performance_lp_rf[["subset-accuracy"]] <-0.13
