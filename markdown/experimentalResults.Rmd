...
Let $U=\{S1\cup S2\cup S3\cup S4\cup S5\cup S6\}$, since the documents labeled with the previous six labels include 86 terms, there was 41 term belong to T but do not belong to U, i.e  $\{|terms|_{term \in \{ T \setminus U \}}=41  term \}$.
...
Classifier chain transformation method delivered poor performance across all classication models(k-nearest neighbour, random forest, XGBoost). XGBoost performed the best with the 

classifier chain, due to the boosting optimization technique the xgboost based on.
